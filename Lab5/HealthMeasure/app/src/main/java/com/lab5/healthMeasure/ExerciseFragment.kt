...
import com.lab5.healthMeasure.databinding.FragmentExerciseBinding
...
class ExerciseFragment : Fragment() {
    ...
    private fun updateExerciseStatus(state: ExerciseState) {
        ...
        if (previousStatus.isEnded && !state.isEnded) {
            // We're starting a new healthmeasure. Clear metrics from any prior healthmeasure.
    ...
    private fun updateChronometer() {
        // We update the chronometer on our own regular intervals independent of the healthmeasure
        // duration value received. If the healthmeasure is still active, add the difference between
